...
// Get reference search term
$reference = trim($_GET['reference'] ?? '');

// Fetch tickets with category_name
if (!empty($reference)) {
    $stmt = $pdo->prepare("
        SELECT t.*, c.category_name
        FROM tickets t
        LEFT JOIN category c ON t.category_id = c.category_id
        WHERE t.reference LIKE :reference
        ORDER BY t.created_at DESC
    ");
    $stmt->execute([':reference' => "%$reference%"]);
} else {
    // Fetch all tickets if no search
    $stmt = $pdo->prepare("
        SELECT t.*, c.category_name
        FROM tickets t
        LEFT JOIN category c ON t.category_id = c.category_id
...
<div class="container-fluid p-4">
    ...
    <div class="card-body">
        <form method="GET" class="mb-3 d-flex gap-2">
            ...
            <a href="ticket_manager.php" class="btn btn-secondary">Reset</a>
        </form>
    </div>
                            ...
                            <?php foreach ($tickets as $ticket): ?>
                                <tr class="ticket-list-item status-<?= strtolower(str_replace(' ', '-', $ticket['status'])) ?>"
                                    ...
                                    <td>
                                        ...
                                        <small class="email"><?= htmlspecialchars($ticket['email']) ?></small>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <span class="badge bg-secondary">
                                            <?= htmlspecialchars($ticket['category_name'] ?? 'No Category') ?>
                                        </span>
                                    </td>
                                    <td>
